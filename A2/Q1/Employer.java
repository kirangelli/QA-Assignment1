...
{
	ArrayList <CalculatePay> calculatePay;
	
	public Employer()
	{
		calculatePay = new ArrayList<CalculatePay>();
		for(int i =0;i<5;i++)
		{
			
			calculatePay.add(new HourlyWorker());
		...
		for(int i =0;i<5;i++)
		{
			...
			calculatePay.add(new SalaryWorker());
	...
	public void outputWageCostsForAllStaff(int hours)
	{
		...
		for(int i =0;i<10;i++)
		{
			CalculatePay calculatePayObject = calculatePay.get(i);
			cost+= calculatePayObject.calculatePay(hours);
		}
